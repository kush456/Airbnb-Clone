...
export default function PhotosUploader({addedPhotos, onChange}) {//iin order to prevent all photos from going if page is reloaded, if we use it as state it is lost 
    const [photoLink, setPhotoLink] = useState('');

    async function addPhotoByLink(ev) {
        ev.preventDefault();//again to prevent the page from reloading upon onChange
    ...
    async function uploadPhoto(ev){
        try {
            const files = ev.target.files;//this prevents the fake path issue/security measure by our browser
    ...
    return (
        <>
            ...
            <div className="mt-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                ...
                <label className="cursor-pointer flex gap-1 mx-1 justify-center border bg-transparent rounded-2xl p-2 text-gray-600 text-xl items-center">
